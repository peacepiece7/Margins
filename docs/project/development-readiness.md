...
- Current latest release deploy: the committed deploy-process update was built into `infra/artifacts/margins-release.zip`, verified, artifact-smoked for backend and frontend, transferred to Raspberry Pi, switched `/opt/margins/current` to a timestamped release under `/opt/margins/releases`, restarted `margins-back`, and passed `http://172.30.1.21/api/health` plus frontend HTTP 200 checks.
