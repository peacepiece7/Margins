...
- Current Raspberry Pi direct live deploy gate: `powershell -NoProfile -ExecutionPolicy Bypass -File infra/scripts/deploy-raspberry-pi.ps1 -SmokeHealthUrl http://172.30.1.21/api/health` passed after `infra/artifacts/margins-release.zip` was built, verified, frontend-smoked, transferred, switched into `/opt/margins/current`, and restarted with post-deploy health smoke.
- Current production env upload: `/opt/margins/.env` is the active Raspberry Pi systemd `EnvironmentFile`; the env file was uploaded with timestamp backup, permission `600`, OpenAI/book-search/runtime DB/JWT keys present, and no secret values printed.
- Current latest release deploy: the AI context, heuristic debate, seed refresh, and operational book-profile backfill updates were built into `infra/artifacts/margins-release.zip`, verified, frontend-smoked, transferred to Raspberry Pi, switched `/opt/margins/current` to a timestamped release under `/opt/margins/releases`, restarted `margins-back`, and passed `http://172.30.1.21/api/health`.
- Current Raspberry Pi schema apply: `infra/scripts/apply-raspberry-pi-schema.ps1` passed against the remote `margins-mysql` Docker container using its existing MySQL environment credentials, applying every `db/schema/*.sql` file including `007_backfill_book_ai_profiles.sql` without applying seed data or printing password values. The follow-up book AI profile gap count query returned `0`.
...
- `npm run deploy:upload-env -- --runtime-env-path .env.production` is the standard production runtime env upload command. It should be run before live deploy when OpenAI, Google Books, DB, JWT, or Spring profile values change.
