...
- Current production env upload: `/opt/margins/.env` is the active Raspberry Pi systemd `EnvironmentFile`; the env file was uploaded with timestamp backup, permission `600`, OpenAI/Kakao/book-search/runtime DB/JWT keys present, and no secret values printed.
