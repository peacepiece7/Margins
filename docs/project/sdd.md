...
- Debate session page where the user chooses a topic, creates a topic-specific conversation room, selects professional reading-lens personas, and discusses that room's topic with the selected personas.
...
Google Books candidates carry the preferred ISBN in both `candidateId` (`google:<isbn>`) and the separate optional `isbn` field. When a Google result does not include ISBN metadata, the backend uses the Google volume id for `candidateId` and leaves `isbn` empty, so the current UI can still save the book without parsing provider-specific data.
...
- Seed personas use professional reading-lens identities for debate: literary critic, philosopher, psychologist, historian, sociologist, editor, skeptical reader, and book-club facilitator. Persona identity remains visible in debate room controls and message bubbles.
...
- External book search uses the configured provider chain. Google Books is the default provider through `MARGINS_BOOK_SEARCH_PROVIDER=google` and `GOOGLE_BOOKS_BASE_URL`; Open Library remains a secondary metadata provider, while AI fallback is controlled by `MARGINS_BOOK_SEARCH_AI_FALLBACK_ENABLED`.
...
페르소나는 전문직 프리셋 중심으로 유지한다. 초기 전문직 후보는 문학평론가, 철학자, 심리학자, 역사학자, 사회학자, 편집자, 회의적인 독자, 독서 모임 진행자다. 전문직 페르소나는 `system_prompt`만이 아니라 관점, 말투, 피해야 할 단정, 응답 패턴을 구조화해 저장한다.
