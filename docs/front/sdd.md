...
- `src/i18n.tsx`: lightweight frontend locale provider, persisted language preference, and Korean translations for fixed UI copy.
...
- The frontend supports English and Korean for fixed application chrome through `I18nProvider`.
- The current locale is stored in `localStorage` under `margins.locale`; absent or unknown values fall back to English.
- `I18nProvider` updates `document.documentElement.lang` so assistive technology and browser language handling follow the selected UI language.
- The login gate and authenticated session bar expose an EN/KO language toggle. The toggle changes UI copy immediately without logging out, clearing auth, or reloading the current reading session.
- User-authored content and backend-provided book, session, message, persona, tag, and insight text are displayed as stored rather than translated.
- English remains the source fallback for missing translations, so feature work can land safely while new fixed UI strings are added to the Korean translation map.
...
- The UI calls `GET /api/personas?sessionId={id}` when a session is known and exposes active personas in the debate form. Session-generated personas are preferred for that reading session; global fallback personas remain available when a session has no scoped personas.
- Readers can create a custom debate persona through `POST /api/personas`; the returned active persona list refreshes the debate selector and selects the new persona for immediate use. Persona models include optional `roleKey` so the UI can label debate roles without making the reader configure them.
- Readers can request persona drafts through `POST /api/personas/generate`; drafts render in the persona composer with save controls, role labels, tone labels, and local UI state until the reader saves one through `POST /api/personas`. The frontend treats backend draft text as already safety-screened; unsafe generated drafts are replaced by the backend before display so no extra setup or moderation step is shown to the reader.
- During default session preparation, generated persona drafts are saved automatically with the new session id so the debate composer is ready without manual persona setup and does not accumulate unrelated book personas as debate targets.
- The question panel can request draft prompts through `POST /api/session-windows/{id}/questions/suggest`; drafts render separately from persisted questions and are kept in local UI state until the reader saves one through `POST /api/session-windows/{id}/questions`.
...
- Message/debate responses are displayed from persisted timeline data, including generated message ids and persona ids. Message history labels non-persona assistant messages as `Book answer`, selected-prompt user messages as `Reader answer`, and persona responses by persona display name, so comprehension turns remain visually distinct from persona debate turns after persistence reload.
- Timeline message models include optional `promptSnapshot` audit metadata from the backend. The frontend preserves the field in curated TypeScript models but does not render it in the reader UI during the MVP.
- Timeline message models and immediate AI response models include optional `tokenUsage` from the backend. The frontend preserves the field for future cost/debug tooling but does not render token counts in the reader UI during the MVP.
- Assistant and persona messages can include `contextSnapshot` from the backend. `SessionWorkbench` parses the snapshot into evidence chips under the message body, showing selected question, persona, saved quote, and recent-message context when available. Malformed or missing snapshots are ignored without blocking message rendering.
- The reading room capture card shows a compact `reading-boundary-warning` when no current page is recorded. The warning focuses the progress area so the reader can set the current page without turning progress tracking into a primary management flow.
...
- The bottom action area renders separate `message-composer` and `persona-composer` regions. The first region is visibly labeled `Ask book` and uses the existing window-message/question contract for comprehension or reflection turns. The second region is visibly labeled `Debate personas` and owns persona creation, single-persona debate, and debate-all controls. Existing form contracts remain unchanged, but the UI no longer presents book questions and persona debate as one generic message area.
- On narrow screens, the bottom action area exposes `composer-mode-tabs` so readers switch between `Ask book` and `Debate personas` instead of scanning both stacked forms. The control uses a standard tab pattern with `aria-selected`, `aria-controls`, and matching `tabpanel` regions. On `xl` and wider screens the tabs are hidden and both composers remain visible side by side for desktop efficiency.
...
- New book selection and saved-book session start auto-prepare the first reading room: after default windows are created, the frontend requests 3 persisted reflection questions and 3 book-context persona drafts, saves those personas, then loads the session timeline. If the AI preparation step fails, the created session still opens and shows a warning instead of forcing the reader to manage setup manually.
- The first viewport includes a compact `reading-workflow` status strip for Add book, Questions, Personas, and Capture. Library dashboard, saved books, memory search, and session history remain available under `library-history-details` so management functions do not dominate the initial reading workflow.
- Active sessions render a `reading-room-board` before window/admin controls. It groups the book room into Questions, Persona cast, Capture, and Discussion cards, showing the first generated prompts, current persona cast with role labels, latest captured passage, and latest persona response. Card actions focus the existing question panel, quote capture, message list, or persona composer so the board is a primary navigation layer over persisted controls rather than a duplicate state owner.
- If an active reading room has no generated questions or no personas, the `reading-room-prepare-retry` action is visible. It retries question generation and requests persona drafts without blocking access to the partially created session.
...
- Question draft generation uses the same focus text as persisted question generation but does not refresh the authoritative timeline until the reader saves a draft.
...
- Persona drafts use the selected book title, reading goal, and session summary as generation context. Saving a draft sends its `roleKey`, removes it from the local draft list after the backend returns the refreshed active persona list, and keeps unsaved drafts inactive.
