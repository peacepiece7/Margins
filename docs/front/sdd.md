...
- The personal reflection composer uses `ReflectionMarkdownEditor`, a lightweight Markdown textarea with formatting controls and preview. Reflection content is persisted as Markdown text, while the display path renders Markdown and keeps a restricted sanitizer only for older CKEditor HTML records.
...
- `book-search` uses `POST /api/books/search-candidates` for external-book candidates with `candidateId`, title, author, optional ISBN, and optional publication year; candidate ids may come from Google Books (`google:`), Open Library (`openlibrary:`), or AI fallback. Google Books candidates may also include optional subtitle, authors, publisher, published date, description, thumbnail, language, page count, ISBN-10, and ISBN-13 metadata. The page can also call `POST /api/books` with a generated `manual-*` candidate id when the user manually enters a book title and author.
- Candidate cards display the backend candidate id as the user-visible localized book identifier (`Book ID` in English, `고유번호` in Korean), plus cover, subtitle, publisher, ISBN, language, and publication year when present. Saving a candidate sends the available provider metadata back to `POST /api/books`.
...
- `book-list` renders `GET /api/books` results as clickable rows with cover, subtitle, publisher, year, source, and ISBN when present. Rows navigate to `book-detail` and expose delete controls.
...
- `review` stores personal reflection notes as `session_insights` with `insightType='reflection'`. The personal reflection composer uses a Markdown-first editor with a large document canvas, formatting shortcuts, preview rendering, and a 16px default body font. Selected question answers and debate messages keep their compact textarea/chat composer behavior. A selected question answer still uses the existing window message stream so the user's answer and AI response remain persisted timeline messages.
...
- Professional personas are displayed in the debate selector. The default seed group includes literary critic, philosopher, psychologist, historian, sociologist, editor, skeptical reader, and book-club facilitator.
...
- Debate participant buttons derive compact markers from professional seeded persona names so the room reads as a set of distinct reading lenses rather than a generic assistant list.
...
- New book selection creates a reading session with a backend-compatible generated title, then creates only a reflection question window. Debate rooms are created later from explicit reader topics, so sessions do not start with an unused default debate room. If the default reflection window call fails after the session has already been created, the UI still loads the created session timeline instead of hiding the partially created session.
