...
- `npm run deploy:cleanup-smoke` runs `db/ops/001_cleanup_production_smoke_data.sql` through the same Raspberry Pi MySQL SSH path. It is intentionally narrower than reset: the SQL matches only `Margins Smoke %` book titles, soft-deletes dependent durable records, and removes derived metrics for those smoke entities. `--dry-run` validates target inputs and prints only the SQL file and prefix scope without opening SSH.
...
- `runtime/env.example` includes `MARGINS_BOOK_SEARCH_AI_FALLBACK_ENABLED=false` and `MARGINS_BOOK_SEARCH_PROVIDER=google`, so external-provider failures are visible during Google Books/Open Library debugging unless operators explicitly opt into AI-generated book candidates.
