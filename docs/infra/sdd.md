...
- `runtime/systemd/margins-back.service.example` starts `/opt/margins/current/back/margins-back.jar` with `EnvironmentFile=/opt/margins/shared/margins.env`, keeping real secrets outside the release artifact.
...
`-SmokeHealthUrl` or `MARGINS_DEPLOY_HEALTH_URL` proves only the configured HTTP health endpoint after the full transfer/restart path succeeds. It does not replace SSH preflight or artifact verification. The expected completion gate is `verify-local-quality.ps1 -DeploymentPreflight -ArtifactRuntimeSmoke -ArtifactFrontendSmoke -SshPreflight -LiveDeploySmoke -DeploySmokeHealthUrl <url>`; the current Raspberry Pi target has passed this gate with `http://172.30.1.21/api/health`.
