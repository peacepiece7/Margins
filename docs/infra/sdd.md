...
| `MARGINS_REMOTE_MYSQL_PASSWORD` | no | Remote database password for schema apply. May fall back to `MARGINS_MYSQL_PASSWORD`; when neither is provided, `apply-raspberry-pi-schema.ps1` uses the MySQL container's existing `MYSQL_PASSWORD` environment internally without printing it. |
