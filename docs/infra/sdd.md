...
- `npm run deploy:pi -- --smoke-health-url <url>` runs an HTTP 2xx/3xx smoke check after a successful full transfer/restart. `MARGINS_DEPLOY_HEALTH_URL` can provide the same value. Dry-run output only reports whether the URL is configured and does not print the URL value; smoke failure output reports only sanitized status/error class information.
