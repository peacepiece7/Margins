...
- `GoogleBooksSearchProvider`: calls Google Books Volumes API and maps `id`, `volumeInfo.title`, `subtitle`, `authors`, `publisher`, `publishedDate`, `description`, `imageLinks`, `language`, `pageCount`, and `industryIdentifiers` into save-compatible candidates. The provider supports Google Books query syntax by preserving `intitle:`, `inauthor:`, and `isbn:` prefixes, translating `title:` and `author:` to the Google operators, and converting ISBN-shaped input to `isbn:{value}`.
...
- `books[]`: `bookId`, `title`, `subtitle`, `author`, `publisher`, `publishedYear`, `isbn`, `source`, `sourceRef`, `coverImageUrl`, `language`

Soft-deleted books are excluded through `books.deleted_at IS NULL`.
`POST /api/books/search-candidates` first calls the configured external provider when `margins.book-search.enabled=true`. `MARGINS_BOOK_SEARCH_PROVIDER=google` makes Google Books the first provider; if it is unavailable or returns no save-compatible records, the backend tries the remaining external providers. The API falls back to `AiProvider.suggestBooks` only when `MARGINS_BOOK_SEARCH_AI_FALLBACK_ENABLED=true`; the default is `false` so Google/Open Library misses return a successful empty candidate list with `aiModel=external-none` instead of being masked by AI candidates or breaking the search UI. Google Books results map preferred ISBN-13 then ISBN-10 to `candidateId` with `google:` prefix and the separate optional `isbn` response field; if ISBN is missing, the Google volume `id` becomes the candidate id. Google also maps title, subtitle, joined authors, authors array, publisher, published date/year, description, thumbnail, language, page count, ISBN-10, and ISBN-13. Open Library results map `key` to `candidateId` with `openlibrary:` prefix, title to title, first `author_name` to author, and `first_publish_year` to `publishedYear`.
Both external and AI paths return candidates that are safe to pass back to `POST /api/books`: blank title or candidate identifier suggestions are removed, missing Google authors are represented as `Unknown author`, returned `candidateId`, `title`, `author`, and optional metadata strings are trimmed and capped at 255 characters, and optional ISBN fields are trimmed and capped at 32 characters.
`POST /api/books` accepts `candidateId`, `title`, and `author` as required non-blank strings up to 255 characters plus optional provider metadata from search candidates: subtitle, authors, publisher, published date/year, ISBN/ISBN-10/ISBN-13, description, thumbnail, language, and page count. It trims title, author, ISBN, language, URLs, and display metadata before persistence and returns the existing saved book instead of inserting when the same user already has a non-deleted book with matching normalized title and author. When a duplicate is reused, provider metadata only fills blank saved-book fields such as ISBN, publisher, source reference, cover URL, and language; user-visible title and author are not overwritten. Candidate ids with a provider prefix, such as `google:` or `openlibrary:`, set `books.source` to that prefix while preserving the full value in `books.source_ref`; unprefixed candidate ids continue to use `source='ai'`. If the mapper reports that no row was inserted for a new book, the backend fails the request with `message=Book could not be saved` and does not return an unsaved record.
...
- Shared outbound HTTP clients are provided through `HttpClientConfig` and injected into OpenAI, Google Books, and Open Library providers instead of constructing a new `HttpClient` per request.
...
- Provider diagnostics: Google Books and Open Library log skipped configuration, non-2xx status, parse-shape issues, unusable records, interruption, exception class, and candidate counts without logging response bodies.
- Google Books endpoint format: `{GOOGLE_BOOKS_BASE_URL}/books/v1/volumes?q={query}&maxResults={limit}&printType=books`. The backend preserves `intitle:`, `inauthor:`, and `isbn:` queries, translates `title:` to `intitle:` and `author:` to `inauthor:`, and converts ISBN-shaped free text to `isbn:{value}`.
