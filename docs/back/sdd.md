...
| `POST` | `/api/books/search-candidates` | Search public catalog book candidates, falling back to AI candidates |
...
| `POST` | `/api/personas/generate` | Generate AI persona drafts without persistence |
...
| `POST` | `/api/session-windows/{id}/questions/suggest` | Generate AI question drafts without persistence |
...
| `POST` | `/api/session-windows/{id}/debate/all` | Request responses from all active personas for one debate prompt |
...
| --- | --- | --- |
| `POST /api/books` | `books` | Uses single-user id `1`, derives `source='open-library'` for `open-library:` candidate ids and `source='ai'` otherwise, and stores `source_ref` from `candidateId`; reuses an existing non-deleted book when normalized title and author match; rejects zero-row inserts instead of returning a fake saved book id. |
...
| `POST /api/personas` | `personas` | Stores an active reader-authored persona with generated internal `name`, display metadata, `system_prompt`, normalized `role_key`, and `is_test_data=true`; rejects zero-row inserts instead of returning an unchanged active persona list as if the create succeeded. Explicit duplicate role keys within the same session return `409`. |
...
| `POST /api/session-windows/{id}/questions/suggest` | none | Validates the target window, calls `AiProvider.suggestQuestions`, and returns draft prompts without inserting `questions` rows. |
...
| `POST /api/session-windows/{id}/messages` | `messages` twice | Stores user message, calls `AiProvider`, then stores final assistant message with `parent_message_id`, `context_snapshot`, and `prompt_snapshot`. `message_order` is calculated within the target `window_id`; persisted `user_id` is resolved from the session-window owner, not from client input. Zero-row inserts fail with `message=Message could not be saved`. |
| `POST /api/session-windows/{id}/messages/stream` | `messages` twice | Streams assistant response chunks as SSE while reusing the same final user/assistant message persistence path and storing prompt metadata with `streaming=true`; persisted `user_id` is resolved from the session-window owner. Zero-row inserts fail with `message=Message could not be saved`. |
| `POST /api/session-windows/{id}/debate` | `messages` twice | Stores user message, calls `AiProvider`, then stores final assistant message with `persona_id`, `context_snapshot`, and `prompt_snapshot`; persisted `user_id` is resolved from the session-window owner. Zero-row inserts fail with `message=Message could not be saved`. |
| `POST /api/session-windows/{id}/debate/all` | `messages` once plus one row per active persona | Accepts `content` without a client `personaId`, stores one user debate prompt, calls `AiProvider` for each active persona, then stores each assistant response linked by `parent_message_id`, `persona_id`, `context_snapshot`, and `prompt_snapshot`; persisted `user_id` is resolved from the session-window owner. Zero-row inserts fail with `message=Message could not be saved`. |
...
`POST /api/books/search-candidates` first asks configured `BookSearchProvider` implementations for public catalog results. The default provider uses Open Library search and returns `candidateId` values prefixed with `open-library:`. If all external providers return no usable candidates or fail, the backend falls back to `AiProvider.suggestBooks`. Returned candidates are safe to pass back to `POST /api/books`: blank title, author, or candidate identifier suggestions are removed, and returned `candidateId`, `title`, and `author` strings are trimmed and capped at 255 characters.
...
- `messages[]`: `messageId`, `sessionId`, `windowId`, `parentMessageId`, `role`, `content`, `messageOrder`, `aiModel`, `personaId`, `questionId`, `promptSnapshot`, `contextSnapshot`, `tokenUsage`, `streamingStatus`
- `nextActions[]`: `actionId`, `label`, `detail`, optional `targetWindowId`, optional `targetQuestionId`

Assistant and persona responses store `messages.context_snapshot` as an AI evidence trace. The JSON snapshot currently uses `schemaVersion=1` and includes the session/window ids plus `references.question`, `references.persona`, `references.highlights[]`, and recent `references.messages[]` records when available. Timeline message DTOs expose this value as `contextSnapshot` so the frontend can show which persisted records were used as response context.

Assistant and persona responses also store `messages.prompt_snapshot` as compact audit metadata. The JSON snapshot currently uses `schemaVersion=1` and includes `promptContractVersion=mvp-answer-v2`, `responseType`, `provider`, `aiModel`, `streaming`, `safetyPolicyVersion`, `groundingPolicyVersion`, and `readingBoundaryPolicyVersion`. Timeline message DTOs and immediate AI responses expose this value as `promptSnapshot`; raw prompts and full OpenAI request bodies remain out of scope for the MVP.

When the OpenAI Responses API returns `usage`, the backend stores that provider JSON as `messages.token_usage` and exposes it as `tokenUsage` on immediate AI responses and timeline message DTOs. Missing usage data remains `null`; placeholder and fallback responses are not required to estimate tokens.

OpenAI prompt context includes a reading boundary derived from `reading_sessions.start_page`, `current_page`, and `target_page`. If no position is recorded, the prompt explicitly says no current reading position is available. If a current page exists, the prompt instructs the model not to reveal or assume content beyond the current page unless the reader already supplied that content through notes or messages.

OpenAI answer prompts use a shared response grounding contract for window answers, streamed window answers, and persona debate. The contract tells the model to ground replies in provided session context, reader quotes, notes, messages, or the selected question; name supporting evidence when possible; state uncertainty when context is insufficient; avoid inventing unavailable book details; and respect the reading boundary.

OpenAI answer and debate responses pass through `AiAnswerQualityPolicy` before the final `AiMessageResponse` is returned. If the generated text omits `Evidence:` or `Uncertainty:` sections, the policy appends short fallback sections so persisted assistant/persona content has a minimum reviewable structure. During SSE streaming, raw provider deltas are still emitted as they arrive; the final persisted `message.done` payload uses the normalized content.
...
It accepts optional `sessionId`; when supplied, generated personas scoped to that session are returned. If a session has no scoped personas, global fallback personas with no `source_session_id` remain available.

Response DTO:

- `personas[]`: `personaId`, `name`, `displayName`, `description`, `tone`, `roleKey`
...
- `roleKey`: optional stable debate role id. Accepted MVP values are `evidence_analyst`, `skeptic`, `connector`, `empathy_reader`, and `style_reader`. Unknown or missing custom values are normalized from persona text; explicit duplicates in the same session return `409` with `message=Persona role already exists for this session`.
- `sessionId`: optional reading session id used to scope generated or custom personas to the current book session.

The backend generates an internal unique `name` from `displayName`, stores the persona as active test-owned MVP data, stores `source_session_id` when `sessionId` is supplied, stores a normalized `role_key`, and returns the active persona list in deterministic id order. If a custom persona omits `roleKey` and the inferred role is already used in the same session, the backend assigns the first unused MVP role key so manual creation stays lightweight. If the mapper reports that no row was inserted, the backend fails the request with `message=Persona could not be saved` instead of returning an unchanged list. Custom personas are immediately available for `POST /api/session-windows/{id}/debate` and are removed by test reset scripts through `personas.is_test_data`.

`POST /api/personas/generate` accepts optional `count`, `bookTitle`, `readingGoal`, and `context`. It calls `AiProvider.suggestPersonas` and returns `PersonaDraftListResponse` with `aiModel` and `personas[]`: `displayName`, `description`, `tone`, `roleKey`, `systemPrompt`, and `reason`. Returned drafts are normalized against the MVP role taxonomy and de-duplicated where possible before they reach the frontend. The endpoint does not insert `personas`; the reader must save a draft through `POST /api/personas` before it becomes active in debate.
Unsafe generated persona drafts are replaced with safe role-based fallbacks rather than failing the whole response. This keeps automatic reading-room preparation available while preventing obviously inappropriate generated persona material from reaching the frontend.

The frontend uses `personaId` in `POST /api/session-windows/{id}/debate`. The backend verifies that the persona is active and non-deleted before storing the debate prompt or response; missing or inactive personas return `404` through the common failure envelope. After validation, the persisted assistant message keeps the same `persona_id` for later trace queries and metrics. `POST /api/session-windows/{id}/debate/all` uses personas scoped to the target session when they exist, falling back to global personas only when no session-generated personas exist.
...
`POST /api/session-windows/{id}/questions/suggest` accepts the same body as generated questions but returns `QuestionListResponse` drafts without persistence. The response may omit generated ids until the frontend saves one draft with `POST /api/session-windows/{id}/questions`.
...
- No RAG in MVP.
- Boundary: `AiProvider`.
- Book search has a separate `BookSearchProvider` boundary. Open Library catalog lookup runs before `AiProvider.suggestBooks`; AI book candidates remain the fallback path.
- Persona drafts use `AiProvider.suggestPersonas` and are never persisted until the caller submits `POST /api/personas`.
- `AiSafetyPolicy` centralizes MVP safety instructions for OpenAI-generated reading app content. `OpenAiAiProvider` appends this policy to book candidate, persona draft, question, answer, stream, and debate instructions so generated text stays respectful, non-harassing, book-grounded, and avoids obvious unsafe categories.
- `PersonaBusiness.generate` screens returned persona drafts through `AiSafetyPolicy.safePersonaDraft`. Drafts containing obvious unsafe markers such as slurs, harassment, explicit sexual content, self-harm encouragement, violent wrongdoing instructions, or graphic violence are replaced with safe role-based fallback drafts before the frontend can display or save them.
- Window answers, streamed window answers, and persona debate replies use `OpenAiAiProvider`'s shared response grounding contract in addition to the safety policy. This is prompt-level enforcement for the MVP; structured response validation remains deferred.
- `AiAnswerQualityPolicy` adds the MVP answer-structure instruction and normalizes final OpenAI answer/debate content so `Evidence:` and `Uncertainty:` sections are present even when the provider omits them.
...
- `OpenAiAiProvider` captures `usage` from JSON responses and streaming completion events when available, preserving the provider shape in `AiMessageResponse.tokenUsage`.
...
- Context included in prompts: current `sessionId`, book title, book author, session title, reading goal, progress note, closeout summary, saved quotes/locations/notes, saved reader insights, persisted questions for the window, recent messages for the session, selected `questionId`, user input, and persona `system_prompt` for debate.
- Persona debate uses the selected persona `system_prompt` plus this book/session/record context and the latest discussion messages, so persona replies can refer to the registered book and existing debate or reader records without RAG.
...
- Events:
  ...
  - `message.done`: final `AiMessageResponse` with `messageId`, `windowId`, `role`, `content`, `streamingReady`, `aiModel`, optional `promptSnapshot`, optional `contextSnapshot`, and optional `tokenUsage`.
