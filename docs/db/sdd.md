...
- Book identity: the application treats a non-deleted book with the same user, normalized title, and normalized author as reusable; `idx_books_user_title_author` supports that lookup. `books.isbn` stores optional provider metadata from selected candidates, including Google Books ISBN values, but duplicate detection does not depend on ISBN during the MVP.
