...
- Book identity: the application treats a non-deleted book with the same user, normalized title, and normalized author as reusable; `idx_books_user_title_author` supports that lookup. `books.isbn`, `books.publisher`, `books.source_ref`, `books.cover_image_url`, `books.language_code`, and `books.raw_metadata.providerMetadata` store optional provider metadata from selected candidates, including Google Books values, but duplicate detection does not depend on ISBN during the MVP.
...
- Professional persona rows remain ordinary `personas` rows so metrics and trace queries can group debate voices through the same `persona_id` dimension.
...
- `db/ops/001_cleanup_production_smoke_data.sql` is the production-only smoke cleanup path. It matches only `books.title LIKE 'Margins Smoke %'`, soft-deletes matching books plus dependent sessions, windows, questions, messages, highlights, tags, and insights, and deletes only derived metrics tied to those smoke entities.
...
- `db/queries/006_book_provider_metadata_gaps.sql` lists external-provider saved books with missing provider metadata and active normalized title/author duplicate groups so operators can inspect Google/Open Library migration fallout without mutating data.
