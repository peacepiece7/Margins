...
$checks = @(
  ...
  @{
    ...
    Evidence = @(
      ...
      Evidence "front/src/store/sessionFlowStore.ts" @("createDefaultSessionPatch", "Session started, but the default window could not be created", "Session started, but library summaries could not be refreshed")
      Evidence "front/src/store/sessionFlowStore.test.ts" @("loads the created session when the default reflection window create fails", "loads the created session when library refresh fails after creation")
      ...
      Evidence ".github/workflows/ci.yml" @("Node harness quality audit", "npm run quality:full -- -SkipBackend -SkipFrontendBuild", "Frontend unit tests", "npm run test:unit", "Frontend build", "Install Playwright Chromium", "npx playwright install chromium", "Frontend production selector check", "npm run verify:production-selectors")
      Evidence "harness/scripts/audit-ci-workflow.ps1" @("CI Workflow Audit", "forbidden deploy pattern", "does not run SSH/SCP, SSH actions, deploy env vars", "deploy-raspberry-pi.ps1", "PASS: CI workflow gates use the Node harness and do not perform live Raspberry Pi deployment.")
