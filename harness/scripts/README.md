...
- Production runtime env upload is handled by `infra/scripts/upload-prod-env.ps1` or `infra/scripts/upload-prod-env.sh`. Both default to `/opt/margins/.env`, back up the existing remote env file, set permission `600`, and print only key names/permissions.
